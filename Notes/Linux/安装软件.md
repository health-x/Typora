...
2.找到镜像工具 --> 镜像加速器 --> 复制加速器地址（填入到docker配置文件中）

3.填入地址的具体配置步骤（四个命令 或者 按照页面上步骤做）
...
# 安装redis（默认安装在/usr/local/bin目录下）
make install

# 启动redis-server测试下(默认前台启动)
./src/redis-server
...
3. 配置文件后台启动
   ...
   - 在任意路径执行：redis-server /opt/redis/redis.conf（使用/opt/redis/redis.conf配置文件启动redis，前提是有这个配置文件）



**修改配置文件**

进入redis解压后的目录，讲redis-conf文件复制到/usr/local/bin目录下，编辑该文件
